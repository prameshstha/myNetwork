...
def HomePostList(request):
    allPosts_and_details = Post.objects.all().order_by('-post_created_at').select_related()
    # allPosts_and_details = Post.objects.raw("""SELECT DISTINCT * FROM `homePage_post`
    #                                             INNER JOIN `auth_user`
    #                                             INNER JOIN `homepage_userdetails`
    #                                             ON (`homePage_post`.`post_user_id` = `auth_user`.`id`
    #                                             && `auth_user`.`id` = `homepage_userdetails`.`user_Id_id`)
    #                                             ORDER BY `homePage_post`.`post_created_at` DESC  """)
    ...
    print(allPosts_and_details.query)
    for all in allPosts_and_details:
        print(all)
    # for qq in queryset:
    #     pid = qq.id
    #     print(qq.id)
    ...
    try:
        userDetail = UserDetails.objects.get(user_Id=user)
    except:
        userDetail = ''
    print(userDetail)
    ...
    context = {
        'form': form,
        'allPosts_and_details': allPosts_and_details,
...
def add_user_detail(request):
    ...
    if add_form.is_valid():
        ...
        gender = add_form.cleaned_data.get('gender')
        pro_pic = add_form.cleaned_data.get('profile_pic')
        cover_pic = add_form.cleaned_data.get('cover_pic')
        new_user = UserDetails(user_DOB=dob, user_gender=gender, user_profile_pic=pro_pic, user_cover_pic=cover_pic)
        new_user.save()
        if new_user is not None:
            new_user = ''
        print(new_user)
...
def ProfileView(request, pk):
    ...
    allPosts_and_details = Post.objects.filter(post_user=userProfile.id)
    ...
    try:
        userDetail = UserDetails.objects.get(user_Id=pk)  # detail of user in profile
    except:
        userDetail = ''
    print('userDetail')
    print(userDetail)
    ...
    friends_and_userDetails = list(chain(friends_profile, allUserDetails))  # merging two query set friends and allUserDetails
    ...
    raw = UserDetails.objects.raw('SELECT * FROM homepage_UserDetails;')
    # abcc = Friends.objects.filter(friends_friend__id=id)
    # print('abcc')
    # print(abcc)
    print('raw')
    print(raw)
